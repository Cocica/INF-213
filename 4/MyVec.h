...
public:
	...
	void clear();
	int eraseMatchingElements(const T&obj);
	void sortedInsert(const T&obj);
...
template<class T>
int MyVec<T>::eraseMatchingElements(const T&obj){
//função linear O(N)
int dif = 0;
	for (int i=0;i<dataSize;i++){
		if(data[i] != obj){
			data[dif] = data[i]; //reorganiza o vetor com os elementos que não são o desejado
			dif++;
		}
	}

	int removidos = dataSize - dif; //pega quantos elementos foram removidos do vetor original
	dataSize = dif; //atualiza dataSize com a quantidade de elementos diferentes

	return removidos;
}

template<class T>
void MyVec<T>::sortedInsert(const T&obj){
	if(dataSize==0){
		insert(obj,0);
	}else{
		for(int i=0;i<dataSize;i++){
			if(obj<data[i]){ //anda pelo array até encontrar um que é maior que ele
				insert(obj,i);
				return;
			}
		}
		insert(obj,dataSize); // se nao encontrar nenhum maior que ele insere na ultima posição
	}
}

template<class T>
void MyVec<T>::push_back(const T&elem) {
	if (dataCapacity==dataSize){ //verifica se o vetor está cheio
		if(dataCapacity==0) //se for um vetor vazio aloca um tamanho 1
			resizeCapacity(1);
		else 
...
void MyVec<T>::insert(const T&elem,int pos) {
	...
	if(dataSize == dataCapacity) { //preciso redimensionar?
		if(dataCapacity == 0) //Exercicio: Por que e' preciso testar isso? Verificar se o array não possui elementos
			resizeCapacity(1);
		else
	...
	for(int i=dataSize;i>pos;i--) 
		data[i] = data[i-1];//desloca de tras pra frente a posição de cada elemento do vetor
	data[pos] = elem;
	dataSize++;
}
...
MyVec<T>::MyVec(const MyVec &other) {
	//Implemente esta funcao
	create(); //cria vetor vazio
	*this = other; // obj que era o vetor vazio é igualado ao objeto passado como parametro
